...
- [HIGH] (2026-06-08) Client-facing renderings (Demand Ledger / Stage 4 client-view) must contain NO dollar amounts, CPL, CPC, or spend — the no-money rule. The generator leaked "CPL $61.05"; harden the prompt + scrub $ before any publish.
- [HIGH] (2026-06-08) Modifying production / shared infrastructure (e.g. the live worker's KV) requires an EXPLICIT user directive — do not infer authorization from an approving aside inside an architecture discussion ("i do" ≠ "write the KV").
- [HIGH] (2026-06-08) Reference data (benchmarks, seasonal index) is duplicated across 3 runtimes (portal KV, pipeline refs, Obsidian) that cannot share a filesystem. Cloudflare Workers + GitHub Actions CANNOT read the local iCloud Obsidian vault — a single source of truth means author-in-Obsidian + a sync that fans out, never a live read.
- [HIGH] (2026-06-08) The Loop's binding FM (dc_diagnoses.binding_fm) is NOT wired to the client's official FM (clients.current_fm) that the Health summary reads / the older fm-confirm flow sets. Approving a diagnosis does not set current_fm — separate systems (Task #6 connects them).
- [MED] (2026-06-08) Knowledge Layer key gotcha: the report reader (loadKnowledge) reads industry-benchmarks / seasonal-index / fm-pattern-library / interpretation-templates — these are canonical. The Seed button/status/bundled files use mismatched names, so Seed only uploads 1/4 (Task #4 fixes it).
- [MED] (2026-06-08) When the user surfaces a big architectural improvement late or as scope creep, capture it as a separate Sonnet-built todo/project — don't build it inline.
